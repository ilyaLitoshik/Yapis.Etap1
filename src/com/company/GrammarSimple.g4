...
ifFunc              : 'if' OpenBracket condition CloseBracket elseifFunc elseFunc?;
bodyFunc            : 'start' ((math EndOfLine) | ifFunc | forFunc)* functionReturn?'finish';
elseifFunc          : 'else if' bodyFunc;
...
ifProc              : 'if' OpenBracket condition CloseBracket elseifProc elseProc?;
elseifProc          : 'else if' bodyProc;
...
math                : (summOperation | diffOperation | multOperation | divOperation | diffOperation | inPowOperation| percentOperation | ass | callProcedure);
operation           : summ | diff | mult | div | inPow | percent | callFunction;
...
percentOperation    : ID Assign (OpenBracket type CloseBracket)? (ID | Number | Digit | operation) Percent (ID | Number | Digit | operation) ;
...
percent             : OpenBracket (ID | Number | Digit | operation) Percent (ID | Number | Digit | operation) CloseBracket;
